...
namespace Okta.Sdk
{
    ...
    public class CustomRetryStrategy : IRetryStrategy
    {
        ...
        public CustomRetryStrategy(int maxRetries, int requestTimeout, int backoffSecondsDelta = DefaultBackoffSecondsDelta, ILogger logger = null)
        {
            ...
            do
            {
                    ...
                    (_requestTimeout <= 0 || stopwatch.Elapsed.Seconds < _requestTimeout))
                {
                    ...
                    if (delayTimeSpan > TimeSpan.Zero)
                    {
                        ...
                        _logger.LogTrace($"Limit: {limit.FirstOrDefault()}, Remaining: {remaining.FirstOrDefault()}, Reset time: {reset.FirstOrDefault()}, Attempt count: {numberOfRetries}");
